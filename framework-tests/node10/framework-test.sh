...
CONTAINER_ID=$(docker ps -a --filter=name=node10-main-container --format "{{.ID}}" --last 1)
echo "CONTAINER_ID=$CONTAINER_ID"
IMAGE_ID=$(docker images --filter=reference=node10-main-container --format "{{.ID}}")
echo "IMAGE_ID=$IMAGE_ID"
EXIT_CODE=$(docker inspect $CONTAINER_ID --format='{{.State.ExitCode}}')

# Sync back the lockfile, in case it changed
if [[ $EXIT_CODE -eq 0 ]]; then
  run_command docker cp $CONTAINER_ID:/framework-test-node10/package-lock.json ./
else
  exit $EXIT_CODE
fi
