...
CONTAINER_ID=$(docker ps -a --filter=name=node12-cjs-main-container --format "{{.ID}}" --last 1)
IMAGE_ID=$(docker images --filter=reference=node12-cjs-main-container --format "{{.ID}}")
...
if [[ $EXIT_CODE -eq 0 ]]; then
  run_command docker cp $CONTAINER_ID:/framework-test-node12-cjs/package-lock.json ./
else
  exit $EXIT_CODE
fi
